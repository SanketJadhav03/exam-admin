...
@section('title', 'Edit Custom Post')
@section('content')
    <div class="container-fluid ">
        <div class="card mt-2">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3>Edit Custom Post</h3>
                <div>
                    <a href="/admin/custom_post" class="btn btn-primary">Back to Custom Posts</a>
                </div>
            </div>
            <div class="card-body ">
                <form action="/admin/custom_post/update" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <input type="hidden" name="id" value="{{ $custom_post->id }}">
                        <div class="col-6">
                                ...
                                <div class="card custom-card shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <div id="previewContainer" class="mt-4 text-center">
                                            @if($custom_post->post_image)
                                                <img id="previewIcon" src="{{ asset('uploads/custom_post/' . $custom_post->post_image) }}" 
                        ...
                        <div class="col-6">
                            <div class="form-group mb-3">
                                ...
                                <select id="select2Basic" class="select2 form-select form-select-lg" name="custom_id" required>
                                    <option value="">Select Custom</option>
                                    @foreach ($customs as $custom)
                                        <option value="{{ $custom->id }}" {{ $custom_post->custom_id == $custom->id ? 'selected' : '' }}>
                                            {{ $custom->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select id="select2Basic" class="select2 form-select form-select-lg" name="language_id" required>
                                    ...
                                    @foreach ($languages as $language)
                                        <option value="{{ $language->id }}" {{ $custom_post->language_id == $language->id ? 'selected' : '' }}>
                                            {{ $language->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control" name="status" required>
                                    <option value="1" {{ $custom_post->status == 1 ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $custom_post->status == 2 ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label">Paid / Free</label>
                                <select class="form-control" name="paid" required>
                                    <option value="1" {{ $custom_post->paid == 1 ? 'selected' : '' }}>Free</option>
                                    <option value="2" {{ $custom_post->paid == 2 ? 'selected' : '' }}>Paid</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary ms-2">
                            <i class="fa fa-save me-2"></i> Update Custom Post
                        </button>
                    </div>
                </form>
            </div>
