    ...
    <div class="container-fluid">
        <div class="card mt-2">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3>Languages</h3>
                <div>
                    <a href="/admin/language/create" class="btn btn-primary">Create Language</a>
                </div>
            </div>
                            ...
                            @forelse ($languages as $index => $language)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($language->image)
                                            <img src="{{ asset('uploads/languages/' . $language->image) }}"
                                                alt="{{ $language->title }}" class="img-thumbnail"
                                                style="width: 60px; height: 60px; object-fit: cover;" data-bs-toggle="modal"
                                                data-bs-target="#imageModal"
                                                data-image="{{ asset('uploads/languages/' . $language->image) }}"
                                    ...
                                    <td class="text- ">
                                        <label class="switch switch-success">
                                            <input type="checkbox" data-id="{{ $language->id }}" class="switch-input  status-toggle" {{ $language->status ? 'checked' : '' }}>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="/language/edit/{{ $language->id }}"
                    ...
                    <div class="row mx-2 mt-3">
                        <div class="col-sm-12 col-md-6">
                            <div class="dataTables_info">
                                Showing {{ $languages->firstItem() }} to {{ $languages->lastItem() }} of {{ $languages->total() }} entries
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-end">
                                    {{ $languages->onEachSide(1)->links('pagination::bootstrap-4') }}
                                </ul>
                            </nav>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="imageModalLabel">Language Image</h5>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.status-toggle').forEach(toggle => {
                toggle.addEventListener('change', function() {
                    ...
                    axios.post('/admin/language/status', {
                            id: languageId,
