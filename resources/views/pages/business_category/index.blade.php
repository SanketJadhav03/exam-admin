    ...
    <div class="container-fluid">
        <div class="card mt-2">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="/admin/business_category/create" class="btn btn-primary">Create Business Category</a>
                </div>
            </div>
            <div class="card-datatable table-responsive">
                ...
                <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                    ...
                    <table class="datatables-products table dataTable no-footer dtr-column" id="DataTables_Table_0"
                        ...
                        <tbody>
                            @forelse ($business_categories as $index => $category)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td class="text-center">
                                        @if ($category->icon)
                                            <img src="{{ asset('uploads/business_category/' . $category->icon) }}"
                                                alt="{{ $category->name }}" class="img-thumbnail"
                                                style="width: 60px; height: 60px; object-fit: cover;" data-bs-toggle="modal"
                                                data-bs-target="#imageModal"
                                                data-image="{{ asset('uploads/business_category/' . $category->icon) }}"
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="/business_category/edit/{{ $category->id }}"
                                                class="btn btn-sm btn-primary me-1 rounded" title="Edit">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <form action="/admin/business_category/{{ $category->id }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="row mx-2 mt-3">
                        <div class="col-sm-12 col-md-6">
                            <div class="dataTables_info">
                                Showing {{ $business_categories->firstItem() }} to {{ $business_categories->lastItem() }} of {{ $business_categories->total() }} entries
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-end">
                                    {{ $business_categories->onEachSide(1)->links('pagination::bootstrap-4') }}
                                </ul>
                            </nav>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.status-toggle').forEach(toggle => {
                toggle.addEventListener('change', function() {
                    ...
                    axios.post('/admin/business_category/status', {
                            id: categoryId,
