    ...
    <div class="container-fluid">
        <div class="card mt-2">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3>Custom</h3>
                <div>
                    <a href="/admin/custom/create" class="btn btn-primary">Create Custom</a>
                </div>
            </div>
                ...
                <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                    ...
                    <table class="datatables-products table dataTable no-footer dtr-column" id="DataTables_Table_0"
                        ...
                        <thead class="border-top">
                            <tr>
                                ...
                                <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                    ...
                                    aria-label="custom: activate to sort column ascending">Title</th>
                        ...
                        <tbody>
                            @forelse ($customs as $index => $custom)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($custom->icon)
                                            <img src="{{ asset('uploads/customs/' . $custom->icon) }}"
                                                alt="{{ $custom->name }}" class="img-thumbnail"
                                                style="width: 60px; height: 60px; object-fit: cover;" data-bs-toggle="modal"
                                                data-bs-target="#imageModal"
                                                data-image="{{ asset('uploads/customs/' . $custom->icon) }}"
                                    ...
                                    <td class="text- ">
                                        <label class="switch switch-success">
                                            <input type="checkbox" data-id="{{ $custom->id }}" class="switch-input  status-toggle" {{ $custom->status ? 'checked' : '' }}>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="/custom/edit/{{ $custom->id }}"
                                                class="btn btn-sm btn-primary me-1 rounded" title="Edit">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <form action="/admin/custom/{{ $custom->id }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center">No customs found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    <div class="row mx-2 mt-3">
                        <div class="col-sm-12 col-md-6">
                            <div class="dataTables_info">
                                Showing {{ $customs->firstItem() }} to {{ $customs->lastItem() }} of {{ $customs->total() }} entries
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-end">
                                    {{ $customs->onEachSide(1)->links('pagination::bootstrap-4') }}
                                </ul>
                            </nav>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.status-toggle').forEach(toggle => {
                toggle.addEventListener('change', function() {
                    ...
                    axios.post('/admin/custom/status', {
                            id: customId,
